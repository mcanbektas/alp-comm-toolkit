/**
 * HDLC (ISO/IEC 13239, ITU-T Q.921 temel/modulo-8 modu) — `hdlcCore.ts`nin
 * (bu dalga, PAYLAŞILAN çekirdek — SDLC de kullanıyor) ÜSTÜNDE ince
 * ProtocolPlugin sarmalı. Çekirdeğin NEDEN kaçışsız bir extractor + basık-mod
 * control field + adlanmamış U-frame M-bit'leri seçtiği `hdlcCore.ts`nin
 * dosya başında.
 */
...
const PROTOCOL_ID = 'hdlc';
/** Protokol adı veridir, çeviriye girmez (CLAUDE.md). */
const PROTOCOL_DISPLAY_NAME = 'HDLC';

const TRANSLATION_KEY_PREFIX = 'protocol.hdlc';
...
/** Address(1) + Control(1) + FCS(2) — Information boş olabilir, bu dördü olamaz. */
...
function parseHdlcFrame(data: Uint8Array, context?: ParseContext): ParseResult {
  ...
  // (flag'ler HARİÇ, `hdlcSyncExtractor`in varsayılanı).
  ...
  const address = byteAt(content, 0);
...
export const hdlcParser: ProtocolParser = {
  ...
  parse(data: Uint8Array, context?: ParseContext): ParseResult {
    return parseHdlcFrame(data, context);
  },
};
...
const EXAMPLE_FRAMES: ExampleFrame[] = [
  {
    id: 'i-frame',
    // Address=0x01, Control=0x42 → I-format, N(S)=1, P/F=0, N(R)=2.
    name: `${TRANSLATION_KEY_PREFIX}.example.iFrame.name`,
    bytes: encodeHdlcSyncFrame(Uint8Array.from([0x01, 0x42, 0xaa, 0xbb])),
    description: `${TRANSLATION_KEY_PREFIX}.example.iFrame.description`,
    expectedValid: true,
  },
  {
    id: 's-frame',
    // Address=0x02, Control=0x71 → S-format, RR (00), P/F=1, N(R)=3.
    name: `${TRANSLATION_KEY_PREFIX}.example.sFrame.name`,
    bytes: encodeHdlcSyncFrame(Uint8Array.from([0x02, 0x71])),
    description: `${TRANSLATION_KEY_PREFIX}.example.sFrame.description`,
    expectedValid: true,
  },
  {
    id: 'u-frame',
    // Address=0x03, Control=0x03 → U-format, M-bit'ler=0, P/F=0.
    name: `${TRANSLATION_KEY_PREFIX}.example.uFrame.name`,
    bytes: encodeHdlcSyncFrame(Uint8Array.from([0x03, 0x03])),
    description: `${TRANSLATION_KEY_PREFIX}.example.uFrame.description`,
    expectedValid: true,
  },
];

export const hdlcPlugin: ProtocolPlugin = {
  id: PROTOCOL_ID,
  ...
  documentation: {
    ...
    layer: 'data-link',
    references: [{ title: 'ISO/IEC 13239 — High-level Data Link Control (HDLC) procedures', url: 'https://www.iso.org/standard/37010.html' }],
  },
