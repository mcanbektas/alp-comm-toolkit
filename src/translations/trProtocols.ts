/**
 * Protokol metinleri — Türkçe. `tr.ts`ten AYRI bir modül ve bu bir PAKET
 * BOYUTU kararıdır, düzen tercihi değil.
 *
 * Ölçüldü (2026-08-30): sözlüğün 6009 anahtarının 4315'i ve 624 kB ham metnin
 * **536 kB'ı** (%86) `protocol.*` altında. Bu metinler yalnız protokol
 * sayfalarında, çözümleme panellerinde ve parser çıktısı basan ekranlarda
 * okunuyor — ana sayfada, hesap araçlarında, dönüştürücüde hiç görünmüyor.
 * Hepsini ilk boyaya bindirmek, kullanıcıların çoğuna hiç açmayacakları
 * sayfaların metnini indirtmekti.
 *
 * Yükleme `translations/index.ts`teki `loadProtocolStrings` ile olur ve
 * ihtiyacı olan ROTALARIN lazy sınırına bağlanmıştır (`AppRouter`): sayfa
 * inerken sözlüğü de iner, o yüzden ekranda "anahtar görünen" bir aralık yok.
 */

export const trProtocols = {
...
/**
 * Protokol sözlüğünün şekli. `enProtocols.ts` bunu ANOTASYON olarak kullanır —
 * `tr.ts`/`en.ts` ikilisindeki `TranslationDictionary` ile aynı disiplin:
 * eksik anahtar ancak anotasyonla derleme hatası olur.
 */
export type ProtocolTranslationDictionary = Record<keyof typeof trProtocols, string>;
